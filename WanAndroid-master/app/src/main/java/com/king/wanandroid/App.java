...
public class App extends Application implements HasActivityInjector {
    ...
    public void onCreate() {
        ...
        appComponent = DaggerAppComponent.builder().appModule(new AppModule(this))
                                         .httpModule(new HttpModule(Constants.BASE_URL)).build();
    ...
    private void cacheUser(User user) {
        SharedPreferencesUtils.getSharedPreferences(this, Constants.KEY_USER).edit()
                              ...
                              .putString(Constants.KEY_ICON, user.getIcon()).putInt(Constants.KEY_TYPE, user.getType())
                              .apply();
    }
    ...
    public User getCacheUser() {
        ...
        if (cache != null && cache.contains(Constants.KEY_USERNAME)) {
            cacheUser = new User();
            cacheUser.setId(cache.getInt(Constants.KEY_ID, 0));
            cacheUser.setUsername(cache.getString(Constants.KEY_USERNAME, null));
            cacheUser.setEmail(cache.getString(Constants.KEY_EMAIL, null));
            cacheUser.setPassword(AES.INSTANCE.decrypt(cache.getString(Constants.KEY_PASSWORD, null)));
            cacheUser.setIcon(cache.getString(Constants.KEY_ICON, null));
            cacheUser.setType(cache.getInt(Constants.KEY_TYPE, 0));
        }
        return cacheUser;
    }
    ...
    public void logout() {
        user = null;
        SharedPreferencesUtils.getSharedPreferences(this, Constants.KEY_USER).edit().clear().apply();
