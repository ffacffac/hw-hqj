package szu.wifichat.android.sql;
...
public class SqlDBOperate
{
    ...
    public UserInfo getUserInfoByID(int id) {
        ...
        // orderBy)
        Cursor cursor = userDataBase.query(userSQLHelper.getTableName(), new String[] { "id", "name", "age", "IMEI",
    ...
    public List<UserInfo> getScrollDataOfUserInfo(int start, int count) {
        ...
        // db = helper.getWritableDatabase();
        Cursor cursor = userDataBase.query(userSQLHelper.getTableName(), new String[] { "id", "name", "age", "sex",
    ...
    public long getCountOfUserInfo() {
        // db = helper.getWritableDatabase();
        Cursor cursor = userDataBase.query(userSQLHelper.getTableName(), new String[] { "count(*)" }, null, null,
                null, null, null);
        if (cursor.moveToNext()) {
    ...
    public List<Integer> getIDOfChattingInfo(int senderID, int receiverID) {
        ...
        Cursor cursor = chatInfoDataBase.query(chatInfoSQLHelper.getTableName(), new String[] { "id" },
     ...
     * 用来获取近期的一系列聊天记录 参数:start为开始位置，count为最大记录数，(倒序排列) 放回List<chattingInfo>
     */
    public List<GroundMessage> getScrollMessageOfChattingInfo(int start, int count) {
        List<GroundMessage> messages = new ArrayList<GroundMessage>();
        Cursor cursor = chatInfoDataBase.query(chatInfoSQLHelper.getTableName(), new String[] { "id", "sendID",
                "receiverID", "chatting", "date" ,"style"}, null, null, null, null, "id desc", start + ","
                + count);
        while (cursor.moveToNext()) {
        	GroundMessage message=chattingInfoToMessage(new ChattingInfo(cursor.getInt(cursor.getColumnIndex("id")), cursor
                    .getInt(cursor.getColumnIndex("sendID")), cursor.getInt(cursor
                    .getColumnIndex("receiverID")), cursor.getString(cursor.getColumnIndex("date")),
                    cursor.getString(cursor.getColumnIndex("chatting")), 
                    cursor.getInt(cursor.getColumnIndex("style"))));
    ...
    private GroundMessage chattingInfoToMessage(ChattingInfo chattingInfo)
    {
    ...
    public long getCountOfChattingInfo() {
        // db = helper.getWritableDatabase();
        Cursor cursor = chatInfoDataBase.query(chatInfoSQLHelper.getTableName(), new String[] { "count(*)" }, null, null,
                null, null, null);
        if (cursor.moveToNext()) {
            long count = cursor.getLong(0);
            cursor.close();
