...
public class UDPSocketThread implements Runnable {
    ...
    private static final int BUFFER_LENGTH = 1024; // 缓冲大小

    private byte[] receiveBuffer = new byte[BUFFER_LENGTH];
    private byte[] sendBuffer = new byte[BUFFER_LENGTH];
            ...
            if (!SessionUtils.isItself(senderIMEI)) { // 过滤自己发送的广播
                switch (commandNo) {
                    ...
                    case IPMSGConst.IPMSG_BR_ENTRY: {
                        ...
                        sendUDPdata(IPMSGConst.IPMSG_ANSENTRY, receiveDatagramPacket.getAddress(), mNearByPeople);
                        Log.i(TAG, "成功发送上线应答");
                    }
                    break;
                    ...
                    case IPMSGConst.IPMSG_SENDMSG: {
                        ...
                        mApplication.sanderIP = senderIp;
                        GroundMessage msg = (GroundMessage) ipmsgRes.getAddObject();
                        ...
                        switch (msg.getContentType()) {
                            case IMAGE:
                                ...
                                sendUDPdata(IPMSGConst.IPMSG_RECIEVE_IMAGE_DATA, senderIp);

                                msg.setMsgContent(BaseApplication.IMAG_PATH + File.separator + msg.getSenderIMEI()
                            ...
                            case VOICE:
                                ...
                                msg.setMsgContent(BaseApplication.VOICE_PATH + File.separator + msg.getSenderIMEI()
                                        + File.separator + msg.getMsgContent());
                                Log.d(TAG, "接收路径:" + msg.getMsgContent());
                                break;
                        ...
                        // mDBOperate.addChattingInfo(senderIMEI, mIMEI, msg.getSendTime(), msg.getMsgContent(), msg
                        //         .getContentType()); // 将聊天记录加入数据库
                ...
                if (receiveDatagramPacket != null) {
                    receiveDatagramPacket.setLength(BUFFER_LENGTH);
                }
            }
        }
    ...
    public void connectUDPSocket() {
        try {
            // 绑定端口
            if (UDPSocket == null) UDPSocket = new DatagramSocket(IPMSGConst.PORT);
            Log.i(TAG, "connectUDPSocket() 绑定端口成功");

            // 创建数据接受包
            if (receiveDatagramPacket == null) receiveDatagramPacket = new DatagramPacket(receiveBuffer, BUFFER_LENGTH);
    ...
    public void stopUDPSocketThread() {
        isThreadRunning = false;
        if (receiveUDPThread != null) receiveUDPThread.interrupt();
    ...
    public void notifyOnline() {
        ...
        //添加在线用户信息
        mNearByPeople = new NearByPeople(mIMEI, avatar, device, nickname, gender, age, constellation, localIPaddress, logintime);
    ...
    /**
     * 判断是否有已打开的聊天窗口来接收对应的数据，如果有则把消息保存到消息列表中
     *
     * @param paramMsg
     * @return
     */
    private boolean isExistActiveActivity(GroundMessage paramMsg) {
