...
public class BaseApplication extends Application {
    ...
    private void createFilePath() {
        if (null == IMAG_PATH) {
            ...
            if (!FileUtils.isFileExists(IMAG_PATH)) FileUtils.createDirFile(BaseApplication.IMAG_PATH);// 如果目录不存在则创建目录
            if (!FileUtils.isFileExists(VOICE_PATH)) FileUtils.createDirFile(BaseApplication.VOICE_PATH);
            if (!FileUtils.isFileExists(FILE_PATH)) FileUtils.createDirFile(BaseApplication.FILE_PATH);
    ...
    public String getOnlineUserIP() {
        if (mOnlineUsers == null || mOnlineUsers.size() == 0) return "";
        Iterator iterator = mOnlineUsers.entrySet().iterator();
        while (iterator.hasNext()) {
            Map.Entry entry = (Map.Entry) iterator.next();
            String key = (String) entry.getKey();
            NearByPeople value = (NearByPeople) entry.getValue();
     ...
     * @param msg       需要缓存的消息对象
     */
    public void addLastMsgCache(String paramIMEI, GroundMessage msg) {
    ...
    public void addUnReadPeople(NearByPeople people) {
        ...
        if (!mUnReadPeople.contains(people)) mUnReadPeople.add(people);
    }
    ...
    public void removeUnReadPeople(NearByPeople people) {
        if (mUnReadPeople.contains(people)) mUnReadPeople.remove(people);
    }
