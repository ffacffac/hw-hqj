...
public interface CommendManager
{
    ...
    /**
     * 读数据 int memBank数据区 int startAddr数据区起始地址（以字为单位） int length要读取的数据长度(以字为单位) byte[]
     * accessPassword 访问密码 返回的byte[] 为 EPC + 读取的数据
     */
    public byte[] readFrom6C(int memBank, int startAddr, int length, byte[] accessPassword);

    /**
     * 写数据 byte[] password 访问密码 int memBank 数据区 int startAddr 起始地址（以WORD为单位） int wordCnt
     * 写入数据的长度（以WORD为单位 1word = 2bytes） byte[] data 写入数据 返回 boolean，true写入数据正确，false写入数据失败
     */
